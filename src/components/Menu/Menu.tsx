...
export default function Menu({ variant = "user", onToggle }: MenuProps) {
  ...
  return (
    <SidebarProvider>
        {/* Sidebar */}
        <Sidebar className={`${isHidden ? "w-20" : "w-56"} bg-white text-black flex flex-col justify-between border-r transition-all duration-300`}>
          <SidebarContent>
            {/* Botón hamburguesa */}
            <div className="flex justify-start p-4">
          <SidebarTrigger className="block" onClick={onToggle}>
            <svg
